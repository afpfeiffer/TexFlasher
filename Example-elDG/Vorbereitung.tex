...
\newcommand{\pagem}[1]{\special{PageMarker=#1}}
...
\begin{sat}[Hopfscher Umlaufsatz ($\mathbb{R}^2$)]\index{Hopfscher Umlaufsatz}\index{Umlaufzahl}
  F"ur jede $C^2$-geschlossene regul"are Kurve mit Bogenl"angenparametrisierung  $\gamma$ gilt $n_\gamma\in \mathbb{Z}$. Falls $\gamma$ einfach geschlossen ist,
  gilt $n_\gamma = \pm 1$.
\begin{proof}
...
Falls $\gamma$ nach Bogenl"ange parametrisiert ist, gilt $||\ddot\gamma||=||\dot e_1||=||\kappa_1\cdot e_2||=\kappa_1$, also stimmen $\kappa$ und $\kappa_1$ f"ur Frenet-Kurven "uberein. Der Satz von Fenchel gilt allerdings bereits f"ur regul"are $C^2$-Kurven.
...
\begin{align*}
 M\cap U^\Phi = \Phi^{-1}((\mathbb{R}^m\times \{0\})\cap V^\Phi)
\end{align*}
...
\pagem{58}
\subsection{Tangentialraum und Erste Fundamentalform}
\pagem{59}
\fc{Dtaratb}
...
\begin{equation*}\begin{aligned}
g^f_x=\left( f^*g \right)_x = g_{f(x)}\left( df_x(\cdot), df_x(\cdot) \right) = \langle df_x(\cdot), df_x(\cdot)\rangle
\end{aligned}\end{equation*}
...
\fc{ZushZwKruemmungsgroessen}
\begin{bem}[Zusammenhang zwischen geod"atischer- und Frenet-Kr"ummung]
 Die geod"atische Kr"ummung spielt f"ur Kurven auf Fl"achen die gleiche Rolle wie die gew"ohnliche Kr"ummung f"ur ebene Kurven. Beispielsweise kann man zeigen, dass es zu vorgegebenem Startpunkt und -vektor und vorgegebener Kr"ummungsfunktion lokal genau und global h"ochstens eine Kurve geben kann. Allerdings stimmt der Hopfsche Umlaufsatz nicht mehr, an diese Stelle tritt sp"ater der lokale Satz von Gau"s-Bonet. Man erh"alt die (Frenet-) Kr"ummung der Raumkurve $\gamma$ mit dem Satz von Meusier wie folgt 
 \[\kappa_{Frenet}=||\ddot\gamma||=\sqrt{\kappa_{geo}^2+h(\dot\gamma,\dot\gamma)^2}\]
...
Sind $X$ und $Y$ keine konstanten Vektorfelder so gilt nicht mehr der Satz v. Schwarz. Sondern wir erhalten f"ur $h:\mathbb R^3\to\mathbb R$
...
\subsection{Die S"atze von Gau"s und Codazzi-Mainardi}
...
seien $X,Y,Z$ tangeltiale Vektorfelder auf $M$. Dann gelten die folgenden Gleichungen.
\begin{enumerate}
...
\end{align*}
insbesondere ist die Gau"skr"ummung 0 in der euklidischen Ebene (Seite 98).
\end{defi}
...
Die zweite Formel benutzen wir, um $K$ f"ur die euklidische, die sphereische und die hyperbolische Geometrie auszurechnen. Es folgt in der euklidischen Ebene wegen $u=1$ schon $K=0$.
...
\begin{proof}
 ...
 $\nabla^\gamma_{\frac{\partial }{\partial t}} \dot\gamma = 0$, die Definition der geod"atischen Linie.
\end{proof}
...
\begin{defi}[Kurvenintegral]
 ...
 \end{align*}
Ist unabh"angig von der Parametrisierung was man mit Hilfe des Transformationssatzes und einer Umparametrisierung einsieht.Es h"angt nicht von der gerichteten Parametrisierung ab, wie man mit Umparametrisierung und Transformationssatz nachrechnen kann! 
\end{defi}
...
Es sei $U\subset \mathbb{R}^2$ \todo{vlt eher $\mathbb{R}^3$?} ein Gebiet mit einer Riemannschen Metrik $g$, und es sei $\gamma:[a,b]\rightarrow U$
